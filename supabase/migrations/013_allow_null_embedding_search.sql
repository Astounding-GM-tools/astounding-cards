-- Migration: Allow Null Embedding Search
-- Purpose: Allow searching community images without an embedding (for empty cards)
-- Date: 2025-12-09
-- Issue: Empty cards couldn't browse community images because embedding was required

-- Function to search images by semantic similarity OR return random images
-- When p_query_embedding is NULL, returns random recent images instead of similarity search
  ...
  IF p_query_embedding IS NULL THEN
    ...
    WITH originals AS (
      ...
      SELECT
        ...
        ci.style,
        ci.created_at,
        0.5::FLOAT AS similarity_score  -- Neutral similarity for random results (explicit FLOAT cast)
...
COMMENT ON FUNCTION search_similar_images IS 'Search community images by semantic similarity using vector embeddings, or return random images if no embedding provided. Requires extensions schema in search_path for vector operators.';
