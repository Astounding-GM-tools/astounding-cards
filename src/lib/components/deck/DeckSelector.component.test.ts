...
describe('DeckSelector Component Integration', () => {
  ...
  it('should test utility functions', async () => {
    ...
    expect(module.getCardSizeInfo('tarot')).toBe('Larger cards with more readable text, 4 cards per page');
    
    // Test sortDecksByLastEdited
    const mockDecks = [
      { id: '1', meta: { lastEdited: 100, name: 'Old', theme: 'classic', cardSize: 'poker' as const, createdAt: 100 }, cards: [] },
      { id: '2', meta: { lastEdited: 200, name: 'New', theme: 'classic', cardSize: 'poker' as const, createdAt: 200 }, cards: [] }
    ];
