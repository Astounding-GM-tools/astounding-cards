...
<div class="preset-selector">
	<div class="section-header">
		...
		{#if showCreateButton}
			<button class="btn btn-secondary btn-sm" onclick={createNewPreset}> ➕ Create Custom </button>
		{/if}
	</div>
		...
		{#if $customPresets.length > 0 || state.showCustom}
			<div class="preset-section">
				<div class="section-toggle">
					...
					<button class="btn btn-ghost btn-sm" onclick={handleToggleCustom}>
						{state.showCustom ? '▲' : '▼'}
					</button>
				</div>
