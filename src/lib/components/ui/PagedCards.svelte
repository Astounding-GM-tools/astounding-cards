...
<script lang="ts">
	...
	import { createPaginatedCards, type Page } from './PagedCards.svelte.ts';
		...
		{#if page.cards.length > 0}
				...
				<div class="card-grid">
					{#each page.cards as card (card.id)}
						<div class="card-wrapper">
							<CardFront {card} theme={$currentDeck.meta.theme} />
						</div>
					{/each}
				</div>
			</div>
			...
			<div class="page" data-size={cardSize}>
				<div class="card-grid back-grid">
					{#each page.cards as card (card.id)}
						<div class="card-wrapper">
							<CardBack {card} theme={$currentDeck.meta.theme} editable={true} />
						</div>
					{/each}
				</div>
			</div>
