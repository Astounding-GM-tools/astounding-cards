...
<dialog bind:this={dialogElement} class="share-dialog" onclose={handleClose}>
		...
		<div class="tab-content">
			{#if state.activeTab === 'share'}
				<div class="share-export" transition:fade>
					<div class="info-section">
						<p>
							Share your deck via URL or export as JSON. Both options require all images to be
							accessible via URLs.
						</p>
							...
							<div class="notices">
								{#if state.migrationNeeded}
									<div class="migration-notice warning">
										...
										<p>
											{state.blobCount} image{state.blobCount === 1 ? '' : 's'} need{state.blobCount ===
											1
												? 's'
												: ''} to be converted to URLs before sharing.
										</p>
										<button
								...
								{#if state.missingImageCount > 0}
									<div class="info-notice">
										...
										<p>
											{state.missingImageCount} card{state.missingImageCount === 1 ? '' : 's'}
											{state.missingImageCount === 1 ? 'has' : 'have'} no image set. {state.missingImageCount ===
											1
												? 'This card'
												: 'These cards'} can still be shared.
										</p>
									</div>
								{/if}
							</div>
			...
			{:else}
				<div class="backup-export" transition:fade>
					<p>
						Create a complete backup including all images. This file can be used to restore your
						deck exactly as it is now.
					</p>
					<button class="primary-button"> Download Backup (.zip) </button>
				</div>
			{/if}
		</div>
	</div>
