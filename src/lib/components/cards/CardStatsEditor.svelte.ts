 ...
 */
export function validateStatForm(
	data: StatFormData,
	existingStats: CardStat[]
): {
	isValid: boolean;
	error?: string;
} {
 ...
 */
export function addStatToArray(
	stats: CardStat[],
	statId: string,
	value: string | number
): CardStat[] {
...
export function updateStatInArray(
	...
	return stats.map((stat) => (stat.statId === statId ? { ...stat, value: newValue } : stat));
}

/**
 * Reorder stats array using drag and drop indices
 */
export function reorderStats(stats: CardStat[], fromIndex: number, toIndex: number): CardStat[] {
 ...
 */
export function getStatLabel(statId: string, definitions: StatDefinition[]): string {
 ...
 */
export function getStatIcon(statId: string, definitions: StatDefinition[]): string {
	const definition = findStatDefinition(statId, definitions);
 ...
 */
export function groupStatsByCategory(stats: StatDefinition[]): Record<string, StatDefinition[]> {
	return stats.reduce(
		(acc, stat) => {
...
export function statsChanged(original: CardStat[], current: CardStat[]): boolean {
	...
	for (let i = 0; i < original.length; i++) {
		if (original[i].statId !== current[i].statId || original[i].value !== current[i].value) {
			return true;
		}
	}
