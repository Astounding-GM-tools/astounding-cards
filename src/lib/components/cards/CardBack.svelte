...
{#if card}
	<CardBase theme={activeTheme}>
		<div class="card-content" class:preview>
			<!-- Content box -->
			<div class="content">
				<!-- Content flourishes -->
				...
				<div class="secrets">
					...
					{#if editable}
						<button class="add-secret" onclick={handleAddSecret} disabled={isSecretsUpdating}>
							{#if isSecretsUpdating}
								Adding...
							{:else}
								Add Secret
	...
	{#if showMechanicsDialog}
		<CardMechanicsDialog {card} onclose={() => (showMechanicsDialog = false)} />
	{/if}
{/if}
