...
{:else}
	<button class="stats-add" class:updating={isUpdating} onclick={openDialog} disabled={isUpdating}>
		{#if isUpdating}
...
<!-- Stats Editor Dialog -->
<dialog bind:this={dialogElement} class="stats-dialog" onclose={() => (isDialogOpen = false)}>
				...
				{#each editorState.editingStats as stat, index}
					...
					>
						...
						<input
							...
							onchange={(e) =>
								updateStatValueHandler(stat.statId, (e.target as HTMLInputElement).value)}
							placeholder="Value"
						/>
						<button
							class="remove-stat"
			...
			<div class="add-stat-form">
				...
				{#if editorState.selectedStatId}
					<div class="stat-value-input">
						<input type="text" bind:value={editorState.newStatValue} placeholder="Enter value" />
				...
				<button
					class="secondary"
					onclick={() =>
						(editorState = updateStatsEditorState(editorState, { showAddStat: false }))}
					>Cancel</button
				>
			</div>
		{/if}
