<script lang="ts">
	...
	const {
		...
		preview,
		cardSize
	}: { children: any; theme?: string; preview?: boolean; cardSize?: string } = $props();

	// Note: CardBase doesn't have access to individual card data, theme priority handled in CardFront
	const state = $derived<CardBaseState>(
		createCardBaseState(theme, preview, cardSize, $currentDeck)
	);
</script>
...
{#key state.activeTheme}
	<div
		class="card-base"
		class:crop-marks={!state.isPreview}
		class:preview={state.isPreview}
