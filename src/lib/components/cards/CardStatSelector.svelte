...
{#if state.displayStat}
	...
	>
		...
		{:else if state.displayStat.statId === 'area' && typeof state.displayStat.value === 'object' && 'type' in state.displayStat.value}
			{formatLocationDisplay(
				state.displayStat.value as { type: 'hard' | 'soft'; value: string },
				locationCards
			)}
		{/if}
	</button>
{:else}
	<button class="stat-add" class:updating={isUpdating} onclick={openDialog} disabled={isUpdating}>
		{#if isUpdating}
			<span class="updating-text">Updating...</span>
		{:else}
			Type
		...
		<div class="options-container">
			...
			<label class="stat-option">
				...
				<div class="stat-details">
					...
					<select disabled={state.formType !== 'item'} bind:value={state.formPortability}>
