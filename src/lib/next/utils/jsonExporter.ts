 ...
 */
export async function exportDeckToJson(deck: Deck, includeBlobs: boolean = false): Promise<JsonExport> {
    if (!includeBlobs) {
 ...
 */
export async function exportDeckAsJsonString(deck: Deck, includeBlobs: boolean = false, indent: number = 2): Promise<string> {
 ...
 */
export async function estimateJsonSize(deck: Deck, includeBlobs: boolean = false): Promise<{ 
    formatted: number; 
    minified: number; 
    readableSize: string;
    minifiedSize: string;
        ...
        cards: [
            {
                ...
                description: "Rich, detailed character description with personality, background, and motivations that bring them to life.",
                image: "https://example.com/character-image.jpg",
                traits: [
                    {
                        title: "Defining Trait",
                        description: "A key characteristic that defines this character's personality or abilities",
                        isPublic: true
                    },
                    {
                        title: "Hidden Secret",
 ...
 */
export async function copyAiPromptToClipboard(theme: string, cardCount: number = 10): Promise<boolean> {
    try {
