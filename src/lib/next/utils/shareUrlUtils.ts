 ...
 */
export function validateShareUrlRoundTrip(deck: Deck): {
	isValid: boolean;
	errors: string[];
	url?: string;
} {
	...
	try {
		...
		if (url.length > 2000) {
			errors.push(
				`Generated URL is very long (${url.length} characters) and may not work in all browsers`
			);
		}
		...
		errors.push(
			`URL round-trip validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
		);
		return { isValid: false, errors };
	}
}
