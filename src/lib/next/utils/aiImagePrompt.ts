...
const IMAGE_SPECS = {
	...
	artStyle:
		'hybrid pencil sketch with selective coloring, mostly grayscale pencil drawing with key features in full color (photorealistic or classical brushwork style), ink-economical design'
} as const;
...
export function generateImagePrompt(card: Card, deckTheme: string = 'fantasy'): string {
	...
	promptParts.push(
		'5:7 aspect ratio, portrait orientation, vertical composition, taller than wide'
	);
	promptParts.push(
		'. Main subject centered in upper half of image, leaving space at bottom for text overlay'
	);
...
function buildSelectiveColoringGuidance(card: Card): string {
	...
	if (isCharacterType(subtitle)) {
		...
		if (
			description.includes('eye') ||
			description.includes('gaze') ||
			description.includes('stare')
		) {
		...
		if (
			...
			description.includes('glow') ||
			description.includes('aura') ||
	...
	// Item-specific coloring guidance
		...
		if (
			description.includes('gem') ||
			description.includes('jewel') ||
...
function buildArtAndTechSpecs(deckTheme: string): string {
	...
	specs.push(
		'The background is pencil sketch, but face, hands and items of importance are highly detailed and in full color'
	);
