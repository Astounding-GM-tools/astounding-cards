...
describe('Share URL Utils - URL Parsing and Import', () => {
	...
	test('importFromUrl returns null for invalid URLs', () => {
		const invalidUrls = ['https://example.com', 'invalid-url', 'https://example.com#data=invalid'];
...
describe('Share URL Utils - URL Length and Utilities', () => {
	...
	test('isShareUrlTooLong detects long URLs', () => {
		const shortDeck = createMockDeck({
			...
			cards: [
				createMockCard({ title: 'A', subtitle: 'B', description: 'C', traits: [], stats: [] })
			]
		});
		...
		const longDeck = createMockDeck({
			meta: {
				...
				description:
					'This is a very long description that contains lots of text to make the URL longer than usual'
			},
			cards: Array.from({ length: 10 }, (_, i) =>
				createMockCard({
					...
					traits: Array.from({ length: 5 }, (_, j) =>
						createMockTrait({
		...
		const longDeck = createMockDeck({
			cards: Array.from({ length: 50 }, (_, i) =>
				createMockCard({
					...
					traits: Array.from({ length: 10 }, (_, j) =>
						createMockTrait({
							title: `Long Trait Title ${j}`,
							description: 'B'.repeat(200)
						})
					),
					stats: Array.from({ length: 10 }, (_, j) =>
						createMockStat({
