		...
		try {
			...
			const mergedDeck: Deck = {
				...
				meta: {
					title:
						deckMetaMergeDecisions.title === 'replace'
							? incomingDeck.meta.title
							: existingDeck.meta.title,
					theme:
						deckMetaMergeDecisions.theme === 'replace'
							? incomingDeck.meta.theme
							: existingDeck.meta.theme,
					layout:
						deckMetaMergeDecisions.layout === 'replace'
							? incomingDeck.meta.layout
							: existingDeck.meta.layout,
			...
			for (const incomingCard of incomingDeck.cards) {
				...
				switch (decision) {
					...
					default:
						console.warn(
							`No decision made for card ${incomingCard.id}, keeping existing or skipping`
						);
				}
			}
		...
		<div class="deck-summary">
			...
			<div class="progress-info">
				<div class="progress-bar">
					<div
						class="progress-fill"
						style="width: {canFinishMerge
							? 100
							: (mergeProgress / incomingDeck.cards.length) * 100}%"
					></div>
				</div>
				<p>Progress: {mergeProgress} / {incomingDeck.cards.length} cards resolved</p>
				{#if totalCards > 0}
					<p class="conflict-summary">
						{conflictedCards.length} conflicts, {newCards.length} new cards
					</p>
				{:else}
			...
			<div class="detailed-merge">
				...
				{#if totalCards === 0}
					<div class="no-card-conflicts">
						...
						<p>
							All cards in the import are identical to existing cards or have been auto-resolved.
						</p>
					</div>
				{:else if currentCard}
