...
<script lang="ts">
	...
	const { mode, attribute, onSave, onCancel, onDelete } = $props<{
		mode: AttributeMode;
	...
	$effect(() => {
		...
		formData.description =
			mode === 'trait' ? attribute.description : (attribute as Stat).description || '';
...
<div class="attribute-editor">
	<div class="editor-content">
		...
		{#if mode === 'stat'}
			<div class="form-row">
				...
				/>
				<label class="checkbox-label">
					<input type="checkbox" bind:checked={formData.tracked} />
					Tracked
				</label>
			</div>
		{/if}
	...
	<div class="editor-actions">
		...
		<div class="action-buttons">
			...
			{/if}
			<button onclick={handleReset} disabled={!hasChanges}> Reset </button>
			...
			<button
				...
				disabled={!hasChanges ||
					!formData.title.trim() ||
					(mode === 'trait' && !formData.description.trim())}
			>
				Save
			</button>
		</div>
