            ...
            if (typeof value[0] === 'object' && value[0] !== null) {
                if (field === 'stats') {
                    ...
                    return value.map(stat => {
                        const props = [];
                        props.push(`${stat.title || 'Unknown'}: ${stat.value !== undefined ? stat.value : 'N/A'}`);
        ...
        <div class="conflict-section">
            ...
            <div class="conflicts-list">
                {#each conflictsByType.added as conflict, cardIndex}
                    {@const cardIndexInFull = cardConflicts ? cardConflicts.findIndex(c => c === conflict) : -1}
                    <div class="conflict-item added" class:resolved={conflict.resolution !== CARD_RESOLUTION.UNRESOLVED}>
                        <div class="card-summary">
                            ...
                            <button
                                class="expand-button"
                                onclick={() => toggleCardExpanded(conflict.cardId)}
                            >
                            ...
                            <div class="card-details">
                                <div class="card-preview">
                                    <div class="preview-field">
                                        <strong>Description:</strong> {formatFieldValue('description', conflict.importedCard.description)}
                                    </div>
                                    {#if conflict.importedCard.prompt}
                                        <div class="preview-field">
                                            <strong>Prompt:</strong> {formatFieldValue('prompt', conflict.importedCard.prompt)}
                                        </div>
                                    {/if}
                                    {#if conflict.importedCard.tags?.length > 0}
                                        <div class="preview-field">
                                            <strong>Tags:</strong> {formatFieldValue('tags', conflict.importedCard.tags)}
                                        </div>
                                    {/if}
                                </div>
                            </div>
        ...
        <div class="conflict-section">
            ...
            <div class="conflicts-list">
                {#each conflictsByType.modified as conflict, cardIndex}
                    {@const cardIndexInFull = cardConflicts ? cardConflicts.findIndex(c => c === conflict) : -1}
                    <div class="conflict-item modified" class:resolved={conflict.resolution !== CARD_RESOLUTION.UNRESOLVED}>
                        <div class="card-summary">
                            <div class="card-info">
                                <h5>{getCardDisplayName(conflict)}</h5>
                                <div class="card-actions">
                                    <label class="resolution-option">
                                        <input
                                            ...
                                            onchange={() => updateCardConflict(cardIndexInFull, CARD_RESOLUTION.KEEP_EXISTING)}
                                        />
                                    ...
                                    <label class="resolution-option">
                                        <input
                                            ...
                                            onchange={() => updateCardConflict(cardIndexInFull, CARD_RESOLUTION.USE_IMPORTED)}
                                        />
        ...
        <div class="conflict-section">
            ...
            <div class="conflicts-list">
                {#each conflictsByType.removed as conflict, cardIndex}
                    {@const cardIndexInFull = cardConflicts ? cardConflicts.findIndex(c => c === conflict) : -1}
                    <div class="conflict-item removed" class:resolved={conflict.resolution !== CARD_RESOLUTION.UNRESOLVED}>
                        <div class="card-summary">
                            <div class="card-info">
                                <h5>{getCardDisplayName(conflict)}</h5>
                                <div class="card-actions">
                        ...
                        {#if expandedCards.has(conflict.cardId) && conflict.existingCard}
                            <div class="card-details">
                                <div class="card-preview">
                                    <div class="preview-field">
                                        <strong>Description:</strong> {formatFieldValue('description', conflict.existingCard.description)}
                                    </div>
                                    {#if conflict.existingCard.prompt}
                                        <div class="preview-field">
                                            <strong>Prompt:</strong> {formatFieldValue('prompt', conflict.existingCard.prompt)}
                                        </div>
                                    {/if}
                                    {#if conflict.existingCard.tags?.length > 0}
                                        <div class="preview-field">
                                            <strong>Tags:</strong> {formatFieldValue('tags', conflict.existingCard.tags)}
                                        </div>
                                    {/if}
                                </div>
                            </div>
