<script lang="ts">
	import type {
		DeckConflict,
		ConflictSummary as SummaryType
	} from '$lib/next/utils/deckMerging.js';
	import {
		METADATA_RESOLUTION,
		CARD_RESOLUTION,
		CARD_CONFLICT_TYPE
	} from '$lib/next/utils/mergeConstants.js';
	...
	let metaConflictsByField = $derived(() => {
		...
		return conflict.metaConflicts.reduce(
			(acc, mc) => {
	...
	let cardStats = $derived(() => {
		...
		const added = conflict.cardConflicts.filter(
			(cc) => cc.type === CARD_CONFLICT_TYPE.ADDED && cc.resolution === CARD_RESOLUTION.ADD
		).length;

		const modified = conflict.cardConflicts.filter(
			(cc) =>
				cc.type === CARD_CONFLICT_TYPE.MODIFIED &&
				(cc.resolution === CARD_RESOLUTION.USE_IMPORTED ||
					cc.resolution === CARD_RESOLUTION.KEEP_EXISTING)
		).length;

		const removed = conflict.cardConflicts.filter(
			(cc) => cc.type === CARD_CONFLICT_TYPE.REMOVED && cc.resolution === CARD_RESOLUTION.REMOVE
		).length;
...
<div class="conflict-summary">
	<div class="summary-header">
		...
		<p class="summary-description">Here's what will change when you import this deck:</p>
	</div>
							...
							<div class="conflict-indicators">
								{#each conflicts as conflict}
									<span
										...
										class:resolved={conflict.resolution !== METADATA_RESOLUTION.UNRESOLVED}
									>
		...
		{#if summary.cardsAdded + summary.cardsModified + summary.cardsRemoved > 0}
			<div class="change-section">
				<div class="section-header">
					...
					<h4>Cards</h4>
					<span class="count"
						>{summary.cardsAdded + summary.cardsModified + summary.cardsRemoved}</span
					>
				</div>
		...
		<div class="preview-content">
			<div class="preview-item">
				...
				<ul>
					...
					<li>
						Latest metadata from {summary.metaChanges > 0
							? 'resolved conflicts'
							: 'current version'}
					</li>
