...
// Helper functions
export function getStatDefinition(statId: string, customStats: StatDefinition[] = []): StatDefinition | undefined {
  return [...COMMON_STATS, ...customStats].find(stat => stat.id === statId);
}

export function getStatsByCategory(customStats: StatDefinition[] = []): { [category: string]: StatDefinition[] } {
