...
<div class="print-container" class:dev-mode={$devMode}>
	...
	<div class="settings no-print">
		<div class="settings-row">
			...
			{#if $currentDeck}
				<button class="action-button" onclick={() => (shareDialog = true)}> 🔗 Share Deck </button>
				<button class="action-button" onclick={addCard}> ➕ Add Card </button>
				<div class="right-controls">
					<button class="action-button info" onclick={() => printDialog?.showModal()}>
	...
	{#if shareDialog && $currentDeck}
		<ShareDialog deck={$currentDeck} onClose={() => (shareDialog = false)} />
	{/if}

	<!-- Deck management dialog -->
	<dialog bind:this={deckDialog as HTMLDialogElement} class="deck-dialog">
		<div class="dialog-header">
			<h2>📚 Deck Management</h2>
			<button class="close-button" onclick={() => deckDialog?.close()}> × </button>
		</div>
