...
export const mockCardOne = {
	...
	stats: [
		{ title: 'Defense', value: 15, tracked: false, isPublic: true },
		{
			...
			tracked: true,
			isPublic: false,
			description: 'Bandits and brigands, all looking for the adventurers.'
		},
		{
			title: 'Area',
			value: 2,
			tracked: false,
			isPublic: true,
			description: '2 miles area coverage'
		},
		{
			title: 'Alert',
			value: 10,
			tracked: true,
			isPublic: false,
			description: 'Failed stealth check? Tick a box. When full, sound the alarm.'
		}
	]
};
...
export const mockCardTwo = {
	...
	description:
		'An ornate compass that responds to supernatural phenomena rather than magnetic north.',
	image: null,
	traits: [
		{
			title: 'Warm brass',
			description: 'Brass and silver construction, always warm to the touch',
			isPublic: true
		},
		{
			title: 'Pointy',
			description: 'Always points in the same direction - but not north.',
			isPublic: true
		},
	...
	stats: [
		{
			...
			isPublic: false,
			description:
				'Discerning collectors would drool all over the Ethereal Compass; a pawnshop might offer 10% of the value of this "decorative, but non-functional compass".'
		},
		{
			title: 'Rarity',
			value: 9,
			tracked: false,
			isPublic: false,
			description: 'Legendary (9/10 scale)'
		},
		{
			title: 'Weight',
			value: 1,
			tracked: false,
			isPublic: true,
			description: 'Light weight (1/10 scale)'
		},
		{ title: 'Health', value: 10, tracked: true, isPublic: true },
		{
			...
			tracked: true,
			isPublic: false,
			description: '1 charge per turn when active. May be charged.'
		}
	]
};
