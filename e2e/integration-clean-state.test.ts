...
test.describe('Complete Integration Tests - Clean State', () => {
	...
	test('complete card creation workflow with markup verification', async ({ page }) => {
		...
		const descField = page
			.locator('textarea[placeholder*="description"], input[value*="Add a description"]')
			.first();
		if (await descField.isVisible()) {
			await descField.fill(
				'A weathered ranger with the bearing of nobility, carrying ancient burdens.'
			);
	...
	test('theme and size changes with visual verification', async ({ page }) => {
		...
		const themeButton = page
			.locator('button:has-text("Theme"), button:has-text("Classic")')
			.first();
		if (await themeButton.isVisible()) {
			await themeButton.click();

			// Switch to Scriptorum theme
			const scriptorumOption = page
				.locator('button:has-text("Scriptorum"), text=Scriptorum')
				.first();
			if (await scriptorumOption.isVisible()) {
				...
				const cardWithTheme = page
					.locator('.card[data-theme="scriptorum"], [data-theme="scriptorum"] .card')
					.first();
		...
		if (await sizeButton.isVisible()) {
			...
			if (await tarotOption.isVisible()) {
				...
				await expect(
					page.locator('.card[data-size="tarot"], [data-size="tarot"] .card')
				).toBeVisible();
